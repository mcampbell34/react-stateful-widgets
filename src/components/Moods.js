...
export default function Moods() {
  const [mood, setMood] = setMood( initialMood, happyMood, sadMood); 
  ...
  const style = {
    fontSize: '1.5em',
    marginBottom: '0.3em',
    color: happyMood ? 'royalblue' : 'crimson' 
  };
  ...
  return (
    <div className='widget-moods container'>
      ...
      <div>
        <button id='makeHappy' onClick= {event => setMood (`${makeHappy}`) }  >Make Happy </button>
        <button id='makeSad' onClick = {event => setMood `${makeSad}`}>Make Sad</button>
        <button id='resetMood' onClick={event => setMood `${reset}`}>Reset</button>
      </div>
    </div>
  );
}
